...
interface SellToCompanyFormProps {
  filterCategory?: "bikes" | "spares";
}

export default function SellToCompanyForm({ filterCategory }: SellToCompanyFormProps) {
  ...
  const filteredVehicles = useMemo(() => {
    ...
    return filtered.filter((v) => `${v.model_code} ${v.engine_number} ${v.chassis_number}`.toLowerCase().includes(q));
  }, [vehicles, query]);
  ...
  const filteredSpares = useMemo(() => {
    ...
    let filtered = spares;
    if (!q) return filtered;
    return filtered.filter((s) => `${s.model_code} ${s.spare_code} ${s.serial_number}`.toLowerCase().includes(q));
  }, [spares, query]);
        ...
        <Card className="border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900/60">
          <CardHeader>
            ...
            <CardDescription className="dark:text-slate-400">
              {filterCategory === "bikes" ? "Showing Vehicles" : "Showing Spare Parts"} - Search and add items to cart.
            </CardDescription>
          </CardHeader>
        ...
        <div className="space-y-6">
          <Card className="border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900/60">
            <CardHeader>
              ...
              <CardDescription className="dark:text-slate-400">Complete company details and totals.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
