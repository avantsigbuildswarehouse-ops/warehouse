        ...
        <div className="flex flex-col gap-3 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-slate-900/60 transition-colors">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Issue multiple units at once using quick select logic.
            </p>
          </div>
        </div>
            ...
            <CardContent className="space-y-4">
              <div className="rounded-xl border border-emerald-200 dark:border-emerald-900/50 bg-white/60 dark:bg-emerald-950/20 p-4">
                <p className="text-sm text-emerald-700 dark:text-emerald-300">
                  <span className="font-semibold">{issueResult.itemCount} {issueResult.itemCount === 1 ? "unit" : "units"}</span> successfully issued to{" "}
                  <span className="font-semibold">{issueResult.targetName}</span>.
                </p>
              </div>
              <Button
                variant="outline"
                onClick={() => setIssueResult(null)}
                className="border-emerald-300 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 dark:hover:bg-emerald-900/30 w-full"
              >
                    ...
                    <div className="space-y-3">
                      ...
                      >
                        ...
                        {targetType === "ASB_Showroom"
                          ? showrooms.map((s) => (
                              <option key={s.showroom_code} value={s.showroom_code}>
                                {s.city} ({s.showroom_code})
                              </option>
                            ))
                          : dealers.map((d) => (
                              <option key={d.dealer_code} value={d.dealer_code}>
                                {d.business_name} ({d.dealer_code})
                              </option>
                            ))}
                      </select>
                    </div>
                    ...
                    <div className="flex flex-col gap-6 md:flex-row md:items-end">
                      ...
                      <div className="space-y-3 w-full md:w-1/3">
                        ...
                        {itemType === "Bike" && availableBikeModels.length === 0 && (
                          <p className="text-xs text-slate-500 dark:text-slate-400">No bikes available</p>
                        )}
                        {itemType === "Spare" && availableSpareTypes.length === 0 && (
                          <p className="text-xs text-slate-500 dark:text-slate-400">No spares available</p>
                        )}
                      </div>
