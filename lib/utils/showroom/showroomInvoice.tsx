...
const generateShowroomInvoicePdf = async (invoiceData: ShowroomInvoiceData, returnPdfData: boolean = false) => {
  const doc = new jsPDF("p", "mm", "a4");

  // Generate document number
  const documentNumber = `SHOWROOM-INV-${invoiceData.showroom_code}-${invoiceData.id || Date.now()}`;
  
  // Check if document already exists (only when not returning PDF data)
  if (!returnPdfData) {
    try {
      const checkResponse = await fetch(`/api/warehouse/showroom-documents?showroom_code=${invoiceData.showroom_code}&document_type=invoice&document_number=${encodeURIComponent(documentNumber)}`);
  ...
  doc.setFontSize(14);
  doc.text("SHOWROOM INVOICE", 105, 20, { align: "center" });

  // Reference Information - Below Logo
  let refY = 40;
  doc.setFontSize(8);
  doc.setFont("helvetica", "normal");
  doc.text("Invoice No:", 15, refY);
  ...
  doc.setFontSize(11);
  doc.setFont("helvetica", "bold");
  doc.text("BILL TO - SHOWROOM", 18, y + 6);
  y += 10;

  const showroomInfo = [
    { label: "Showroom Code", value: invoiceData.showroom_code || "-" },
  ...
  const fileName = `Showroom_Invoice_${invoiceData.showroom_code}_${invoiceData.id}.pdf`;
  doc.save(fileName);
  ...
  try {
    const saveResponse = await fetch("/api/warehouse/showroom-documents", {
      ...
      body: JSON.stringify({
        showroom_code: invoiceData.showroom_code,
...
export default generateShowroomInvoicePdf;