...
const generateCustomerQuotationPdf = async (
  ...
  const documentPrefix = quotationData.document_number_prefix || "CUSTOMER-QUOT";
  ...
  doc.setFontSize(14);
  doc.text(quotationData.document_title || "QUOTATION", 105, 20, { align: "center" });
  ...
  doc.setFontSize(8);
  doc.setFont("helvetica", "normal");
  doc.text(quotationData.document_label || "Quotation No:", 15, refY);
  doc.text("Date:", 15, refY + 6);
  doc.text("Valid Until:", 15, refY + 12);
  ...
  doc.setFontSize(8);
  doc.text(formatDate(new Date(quotationData.created_at || Date.now())), 55, refY + 6);
  const validUntil = new Date();
  validUntil.setDate(validUntil.getDate() + 30);
  doc.text(formatDate(validUntil), 55, refY + 12);
