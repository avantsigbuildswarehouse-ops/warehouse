...
const generateCompanyDeliveryNotePdf = async (
  deliveryData: CompanyDeliveryData,
  returnPdfData: boolean = false
) => {
  const doc = new jsPDF("p", "mm", "a4");
  const documentNumber = `COMPANY-DN-${deliveryData.target_code || "SALE"}-${deliveryData.id || Date.now()}`;

  if (!returnPdfData && deliveryData.company_id) {
    try {
      const exists = await checkExistingSalesDocument({
        endpoint: "/api/sales/company-documents",
        idKey: "company_id",
        idValue: deliveryData.company_id,
  ...
  if (deliveryData.company_id) {
    await saveSalesDocumentReference({
      endpoint: "/api/sales/company-documents",
      buyerKey: "company_id",
      buyerId: deliveryData.company_id,
      documentType: "delivery_note",
      documentNumber,
      documentData: {
        ...deliveryData,
