...
const generateDealerDeliveryNotePdf = async (deliveryData: DealerDeliveryData, returnPdfData: boolean = false) => {
  const doc = new jsPDF("p", "mm", "a4");

  // Generate document number
  const documentNumber = `DEALER-DEL-${deliveryData.dealer_code}-${deliveryData.id || Date.now()}`;
  
  // Check if document already exists (only when not returning PDF data)
  if (!returnPdfData) {
    try {
      const checkResponse = await fetch(`/api/warehouse/dealer-documents?dealer_code=${deliveryData.dealer_code}&document_type=delivery_note&document_number=${encodeURIComponent(documentNumber)}`);
  ...
  doc.setFontSize(11);
  doc.setFont("helvetica", "bold");
  doc.text("DELIVERY TO - DEALER", 18, y + 6);
  y += 10;

  const dealerInfo = [
    { label: "Dealer Code", value: deliveryData.dealer_code || "-" },
  ...
  const fileName = `Dealer_DeliveryNote_${deliveryData.dealer_code}_${deliveryData.id}.pdf`;
  doc.save(fileName);
  ...
  try {
    const saveResponse = await fetch("/api/warehouse/dealer-documents", {
      ...
      body: JSON.stringify({
        dealer_code: deliveryData.dealer_code,
...
export default generateDealerDeliveryNotePdf;