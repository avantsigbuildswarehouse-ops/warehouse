...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { customer_id, document_type, document_number, document_data } = body;

    if (!customer_id || !document_type || !document_number || !document_data) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: customer_id, document_type, document_number, document_data",
        },
        { status: 400 }
      );
    }
