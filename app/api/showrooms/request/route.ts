...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { showroomCode, itemType, items, remarks } = body;

    if (!showroomCode || !itemType || !Array.isArray(items) || !items.length) {
    ...
    if (itemType === "Bike") {
      return handleBikeRequest(supabase, showroomCode, cleanItems, remarks, requestedAt);
    }

    if (itemType === "Spare") {
      return handleSpareRequest(supabase, showroomCode, cleanItems, remarks, requestedAt);
    }
...
async function handleBikeRequest(
  ...
  const referenceNo = createBatchReference("REQ-SHW-V");
...
async function handleSpareRequest(
  ...
  const referenceNo = createBatchReference("REQ-SHW-S");

  // 1. Fetch available spares
