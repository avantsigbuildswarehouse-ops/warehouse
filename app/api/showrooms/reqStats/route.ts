...
export async function GET(req: Request) {
  try {
    ...
    const showroomCode = url.searchParams.get("showroomCode");

    if (!showroomCode || showroomCode === "undefined") {
      return NextResponse.json({ error: "Missing showroomCode" }, { status: 400 });
    }
